...
def train(output_dir, data_dir, class_list_file, learning_rate, batch_size, iterations, checkpoint_period, device,
    ...
    cfg = get_cfg(output_dir=output_dir, learning_rate=learning_rate, batch_size=batch_size, iterations=iterations,
                  checkpoint_period=checkpoint_period, model=model, device=device, nmr_classes=nmr_classes)
    cfg.merge_from_file(model_zoo.get_config_file(model))
    cfg.DATASETS.TRAIN = ("train",)
